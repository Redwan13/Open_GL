...
float UFO_2_X_pos[1000] = {800};
float UFO_2_Y_pos[1000] = {0};
float UFO_2_X2_pos = 128;
float UFO_2_Y2_pos = 128;
...
void myInit(void)
{
    ...
    imageloader *background = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/mainmenu.bmp");
    imageloader *game_background = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/game.bmp");
    imageloader *instruction = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/Instruction.bmp");
    imageloader *pause = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/pause.bmp");
    imageloader *spaceship = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/spaceship_5.bmp");
    imageloader *enemy_1 = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/UFO_1.bmp");
    imageloader *enemy_2 = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/UFO_2.bmp");
    imageloader *bullet = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/Bullet.bmp");
    imageloader *lifebar = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/life_bar_2.bmp");
    imageloader *gameover = loadBMP("E:/Space-Invades-Computer-Graphics-C-OpenGL--master/assets/gameover.bmp");
...
void enemy_2(float UFO_1_X_pos, float UFO_1_Y_pos)
{
    glEnable(GL_TEXTURE_2D);
    ...
    else if (start == true && gameover == false)
    {
        ...
        else
        {
            if (!pause)
            {
                ...
                // How fast enemy_2 / UFO 2 will go
                for (int i = 0; i < enemy_2_count; i++)
                {
                    // Increasing Y co. of UFO
                    UFO_2_Y_pos[i] += 0.4;
                    // Then creating new Enemy with new indexed of new coordinates
                    enemy_2(UFO_2_X_pos[i], UFO_2_Y_pos[i]);
                }
                ...
                // For Enemy 2
                if (delay_time_2 > 5000)
                {
                    // DEfinig the new coordinates a new enemy in new index of UFO array with initial coordinates
                    UFO_2_X_pos[enemy_2_count] = 800;
                    UFO_2_Y_pos[enemy_2_count] = 0;
                    enemy_2_count++;
                    delay_time_2 = 0;
                }
                delay_time++;
                ...
                // Collision between bullet and UFO                // Collision between bullet and UFO
                for (int i = 0; i < fire_count; i++)
                {
                ...
                // Collision between bullet and UFO_2
                for (int i = 0; i < fire_count; i++)
                {
                    for (int j = 0; j < enemy_2_count; j++)
                    {
                        // Perfect collision
                        // If U_X_min < B_X_min < U_X_max      and     U_Y_min < B_Y_max < U_Y_max
                        if ((UFO_2_X_pos[j] - 10 < bullet_X_arr[i] && bullet_X_arr[i] < UFO_2_X_pos[j] + 128 && UFO_2_Y_pos[j] < bullet_Y_arr[i] + 25 && bullet_Y_arr[i] + 25 < UFO_2_Y_pos[j] + 128) || (UFO_2_X_pos[j] - 10 < bullet_2_X_arr[i] && bullet_2_X_arr[i] < UFO_2_X_pos[j] + 128 && UFO_2_Y_pos[j] < bullet_2_Y_arr[i] + 25 && bullet_2_Y_arr[i] + 25 < UFO_2_Y_pos[j] + 128))
                        {
                            clash_count++;
                            fire_count--;
                            enemy_2_count--;
                            ...
                            // To vanish blusted UFO, we are replacing the current UFO to next UFO
                            for (int b = j; b < enemy_2_count; b++)
                            {
                                // cout << "UFO_1_Y_pos[b]: " << UFO_1_Y_pos[b] << endl;
                                // cout << "UFO_1_Y_pos[b + 1]: " << UFO_1_Y_pos[b + 1] << endl;
                                UFO_2_X_pos[b] = UFO_2_X_pos[b + 1];
                                UFO_2_Y_pos[b] = UFO_2_Y_pos[b + 1];
                            }
                            // PlaySound(TEXT("F:/Study/Semester 9/Computer Graphics/Project/Space Invaders/assets/sound/Explosion3.wav"), NULL, SND_ASYNC);
                        }
                    }
                ...
                for (int i = 0; i < enemy_count; i++)
                {
                    ...
                    // if ((spaceship_X_min < UFO_2_X_pos[i] && UFO_2_X_pos[i] < spaceship_X_min + spaceship_X_max && spaceship_Y_min + spaceship_Y_max < UFO_2_Y_pos[i] && UFO_2_Y_pos[i] < spaceship_Y_min + spaceship_Y_max + 0.1) || (spaceship_X_min < UFO_2_X_pos[i] + 128 && UFO_2_X_pos[i] + 128 < spaceship_X_min + spaceship_X_max && spaceship_Y_min + spaceship_Y_max < UFO_2_Y_pos[i] && UFO_1_Y_pos[i] < spaceship_Y_min + spaceship_Y_max + 0.1))
                    // {
                    //     spaceship_clash++;
                    //     // Explosion Sound
                    //     // PlaySound(TEXT("F:/Study/Semester 9/Computer Graphics/Project/Space Invaders/assets/sound/Explosion3.wav"), NULL, SND_ASYNC);
                ...
                // collision between spaceship and UFO 2
                for (int i = 0; i < enemy_2_count; i++)
                {
                    if ((spaceship_X_min < UFO_2_X_pos[i] && UFO_2_X_pos[i] < spaceship_X_min + spaceship_X_max && spaceship_Y_min + spaceship_Y_max < UFO_2_Y_pos[i] && UFO_2_Y_pos[i] < spaceship_Y_min + spaceship_Y_max + 0.1) || (spaceship_X_min < UFO_2_X_pos[i] + 128 && UFO_2_X_pos[i] + 128 < spaceship_X_min + spaceship_X_max && spaceship_Y_min + spaceship_Y_max < UFO_2_Y_pos[i] && UFO_2_Y_pos[i] < spaceship_Y_min + spaceship_Y_max + 0.1))
                    {
                        spaceship_clash += 2;
                        // Explosion Sound
                        // PlaySound(TEXT("F:/Study/Semester 9/Computer Graphics/Project/Space Invaders/assets/sound/Explosion3.wav"), NULL, SND_ASYNC);
